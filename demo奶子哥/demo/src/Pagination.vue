<template>
  <div class="pagination-container">
    ...
    <span
        :class="[{ 'active': modelValue === item.text }, item.type]"
...
export default {
  ...
  methods: {
    ...
    handlePut(type) {
      ...
      let currentPage = this.modelValue === this.pageTotal? this.pageTotal : 1
      //上一页
      if (type === 'prev' && this.modelValue !== 1) {
        currentPage = this.modelValue - 1
      }
      //下一页
      if (type === 'next' && this.modelValue !== this.pageTotal) {
        currentPage = this.modelValue + 1
      }
      //向前跳转5页
      if (type === 'prevTo') {
        currentPage = Math.max(1, this.modelValue - 5)
      }
      //向后跳转5页
      if (type === 'nextTo') {
        currentPage = Math.min(this.pageTotal, this.modelValue + 5)
      }
