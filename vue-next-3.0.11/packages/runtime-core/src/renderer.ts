...
function baseCreateRenderer(
  ...
  const patch: PatchFn = (
    n1, // n1 表示旧的vnode, 当n1位null时就表示是一次挂载(挂载还是跟新是n1决定的)
  ...
  const mountElement = (
    ...
    if (
      ...
      // 所以最终不管是vue还是react, 他们还是会操作dom, 只是对我们都是不可见的
      ...
      // 4.如果子节点是一个数组的情况
        ...
        // 调用mountChildren来处理子节点的children
        // 另外注意: 这里传入的container其实是el, el是上面刚刚创建的element对象, 这样就建立了父子关系
        mountChildren(
  ...
  const mountChildren: MountChildrenFn = (
    ...
    for (let i = start; i < children.length; i++) {
      ...
      // 调用patch如果继续有子节点就会一次执行(其实是一个深度优先的算法)
      patch(
        null,
        child,
        container,
  ...
  const mountComponent: MountComponentFn = (
    ...
    // 对template进行编译
    setupComponent(instance) // 初始化component
    if (__DEV__) {
  ...
  const updateComponent = (n1: VNode, n2: VNode, optimized: boolean) => {
    ...
    // 判断是否需要更新组件
    // 对比组件vnode的props/children/dirs/transition等属性, 来决定是否需要更新(属性变化, 那么就是需要更新)
    ...
    if (shouldUpdateComponent(n1, n2, optimized)) {
      ...
      if (
        ...
        // normal update
        // 新的子组件vnode赋值给instance的next
        instance.next = n2
        // in case the child component is also queued, remove it to avoid
        // double updating the same child component in the same flush.
        // 防止子组件被添加到更新队列中, 防止重复的更新(double updating)
        invalidateJob(instance.update)
        // instance.update is the reactive effect runner.
        // 执行子组件的副作用渲染渲染函数(update)(主动调用)
        instance.update()
      }
    } else {
  ...
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    // 创建响应式的副作用渲染函数
    // effect的作用是当组件的数据发生变化时, effect函数包裹的内部函数componentEffect会重新执行一遍, 重新渲染组件
    instance.update = effect(function componentEffect() {
      ...
      if (!instance.isMounted) {
        ...
        // 从而获取到 子树的vnode
        // renderComponentRoot中还有收集dynamicBlock
        ...
        // 而是等到整个都渲染完毕的时候，才会在render函数执行完毕后执行flushPostFlushCbs来执行mounted
        // to 2394行，render函数内部
        if (m) {
  ...
  const patchChildren: PatchChildrenFn = (
    ...
    // fast path
    if (patchFlag > 0) {
      if (patchFlag & PatchFlags.KEYED_FRAGMENT) { // 有key, 那么执行patchKeyedChildren
        ...
        return
      } else if (patchFlag & PatchFlags.UNKEYED_FRAGMENT) { // 没有key, 那么执行patchUnkeyedChildren
        // unkeyed
