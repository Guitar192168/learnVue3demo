...
export const PublicInstanceProxyHandlers: ProxyHandler<any> = {
  get({ _: instance }: ComponentRenderContext, key: string) {
    ...
    let normalizedProps
    if (key[0] !== '$') {
      const n = accessCache![key] // 先从缓存看看有没有值
      if (n !== undefined) { // 第一次是没值的
        switch (n) {
      ...
      } else if (setupState !== EMPTY_OBJ && hasOwn(setupState, key)) { // 先判断了setUp
        accessCache![key] = AccessTypes.SETUP
        return setupState[key] // 直接return
        ...
        // props
        (normalizedProps = instance.propsOptions[0]) && //props
        hasOwn(normalizedProps, key)
      ) {
        accessCache![key] = AccessTypes.PROPS
        return props![key]
      } else if (ctx !== EMPTY_OBJ && hasOwn(ctx, key)) {  // ctx  包括computed,methods
